...
public class CreateShortcutActivity extends ListActivity
		implements DialogInterface.OnClickListener, Dialog.OnCancelListener {
	...
	public void onCreate(Bundle savedState) {
		...
		setListAdapter(ArrayAdapter.createFromResource(this, R.array.mainMenu, 1));
	}
	...
	protected void onListItemClick(ListView list, View view, int position, long id) {
		...
		case LIST_ITEM_DIRECT_CALL: {
			...
			intent.putExtra(Contacts.Intents.UI.TITLE_EXTRA_KEY, getText(R.string.callShortcutActivityTitle));
		...
		case LIST_ITEM_DIRECT_TEXT: {
			Intent intent = new Intent(Intent.ACTION_PICK, Phones.CONTENT_URI);
			intent.putExtra(Contacts.Intents.UI.TITLE_EXTRA_KEY, getText(R.string.textShortcutActivityTitle));
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent result) {
		...
		switch (requestCode) {
		case REQUEST_PHONE: {
			startShortcutEditor(generatePhoneShortcut(result, R.drawable.sym_action_call, "tel", Intent.ACTION_CALL));
			break;
		}

		case REQUEST_TEXT: {
			startShortcutEditor(
					generatePhoneShortcut(result, R.drawable.sym_action_sms, "smsto", Intent.ACTION_SENDTO));
			break;
		}
	...
	private Intent generatePhoneShortcut(Intent result, int actionResId, String scheme, String action) {
		...
		Cursor cursor = getContentResolver().query(phoneUri,
				new String[] { Phones.PERSON_ID, Phones.DISPLAY_NAME, Phones.NUMBER, Phones.TYPE }, null, null, null);
		try {
			cursor.moveToFirst();
	...
	/**
	 * Generates a phone number shortcut icon. Adds an overlay describing the type
	 * of the phone number, and if there is a photo also adds the call action icon.
	 *
	 * @param personUri
	...
	private Bitmap  generatePhoneNumberIcon(Uri personUri, int type, int actionResId) {
		...
		int iconSize = (int) r.getDimension(1);
